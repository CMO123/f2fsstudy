...
{//得到checkpoint中data summary的起始块地址,返回值为513
//pr_notice("start_cp_addr = 0x%x\n", __start_cp_addr(sbi));// cp_addr = 512
	return __start_cp_addr(sbi) +
		le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_start_sum);
}

static inline block_t sum_blk_addr(struct f2fs_sb_info *sbi, int base, int type)
{//获得checkpoint中特定sum_blk的起始地址
//pr_notice("cp_pack_total_block_count = 0x%x\n",le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count));
//pr_notice(" __start_cp_addr(sbi) +	le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count)	- (base + 1) + type = 0x%x\n", __start_cp_addr(sbi) +
//		le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count)
//				- (base + 1) + type);

	return __start_cp_addr(sbi) +
		le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count)
				- (base + 1) + type;// cp_pack_total_block_count = 0x6
}
