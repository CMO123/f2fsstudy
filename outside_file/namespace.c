			...
			int mnt_flags, const char *name, void *data)
{
	...
	if (!fstype)
		return -EINVAL;
	///*根据"mount -t jffs2 "指定的文件系统类型jffs2找到jffs2_fs_type *
...
{//首先获得挂载点目录的dentry结构以及目的文件系统的vfsmount结构
...
// data_page: options
	...
	//kern_path()找到挂载点的目录项,即此时path.dentry->d_iname=test;
		...
		char __user *, type, unsigned long, flags, void __user *, data)
{
	...
	kernel_type = copy_mount_string(type);//f2fs

	ret = PTR_ERR(kernel_type);
	if (IS_ERR(kernel_type))
		goto out_type;

	kernel_dev = copy_mount_string(dev_name);// /dev/nvme0n1
