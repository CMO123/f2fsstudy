...
{//为sbi->sm_info->fcc创建结构，并初始化flush_thread
...
{//为sm_info创建discard_cmd_control,并初始化discard_线程
	dev_t dev = sbi->sb->s_bdev->bd_dev;
...
{//计算当前summary所需pages数目
	int valid_sum_count = 0;
	int i, sum_in_page;

	//统计hot,warm,cold data总共写入了多少个block，则需要多少个summary entry数目，如果小于一个页所能装载的summary_entry数目，则返回1个page即可。
	for (i = CURSEG_HOT_DATA; i <= CURSEG_COLD_DATA; i++) {//for循环，valid_sum_count = 0，1，1
		//pr_notice("valid_sum_count = 0x%x\n", valid_sum_count);
...
//pr_notice("valid_sum_count=0x%x\n",valid_sum_count); valid_sum_count = 0x1
	sum_in_page = (PAGE_SIZE - 2 * SUM_JOURNAL_SIZE -
			SUM_FOOTER_SIZE) / SUMMARY_SIZE;	// sum_in_page = 0x1b7 = 439
//pr_notice("sum_in_page = 0x%x\n", sum_in_page);
...
{//从checkpoint中读取summary block，并将数据恢复到seg_i->journal，和curseg_info->sum_blk的summary block中缓存下来。包括nat cache、sit cache和summary entry
	...
	start = start_sum_block(sbi);// start = 0x201 = 513
//pr_notice("checkpoint data summay block addr start = 0x%x\n",start);
...
//pr_notice("blk_addr = 0x%x\n", blk_addr);
	...
	if (IS_NODESEG(type)) {
		...
		} else {//重新从node中一个一个扫描读取，感觉是在por(power-off-recovery)的时候才会扫描整个segment
...
{//获取curseg_summary信息
	...
	if (is_set_ckpt_flags(sbi, CP_COMPACT_SUM_FLAG)) {//进入
		int npages = npages_for_summary_flush(sbi, true);	//npages = 1
		...
		read_compacted_summaries(sbi);//从checkpoint中恢复nat cache，sit cache，summary entry
		type = CURSEG_HOT_NODE;
	}

	if (__exist_node_summaries(sbi)){
		//pr_notice("sum_blk_addr=0x%x\n",sum_blk_addr(sbi, NR_CURSEG_TYPE, type));
		//pr_notice("NR_CURSEG_TYP - type=0x%x\n",NR_CURSEG_TYPE - type);
		ra_meta_pages(sbi, sum_blk_addr(sbi, NR_CURSEG_TYPE, type),
						NR_CURSEG_TYPE - type, META_CP, true);// sum_blk_addr = 0x202 = 514 , NR_CURSEG_TYPE - type = 3

	}
	//从checkpoint中读取hot，warm，cold的node summary
...
{//从journal中查找nid == val，返回journal第i项
	int i;
...
{//创建sbi->sm_info->sit_info,创建sit_i->sentries缓存segments的seg_entry信息，创建sit的bitmap，初始化sit信息， 如操作函数，sit地址，block等
	...
	//2. sit segments层的cache，缓存每个segments的seg_entry信息
	sit_i->sentries = f2fs_kvzalloc(sbi, MAIN_SEGS(sbi) *
					sizeof(struct seg_entry), GFP_KERNEL);//kvzalloc会在分配到的内存返回之前进行清零操作;
	if (!sit_i->sentries)
		return -ENOMEM;
	...
	for (start = 0; start < MAIN_SEGS(sbi); start++) {
		...
		if (f2fs_discard_en(sbi)) {//discard是否能用
	...
	/*3. get information related with SIT */
	sit_segs = le32_to_cpu(raw_super->segment_count_sit) >> 1;//raw_super->segment_count_sit) = 0x2,sit_seg = 1
//pr_notice("raw_super->segment_count_sit = 0x%x\n",raw_super->segment_count_sit);
//pr_notice("sit_segs = 0x%x\n",sit_segs);

	/* setup SIT bitmap from ckeckpoint pack */
	//4. 从checkpoint处建立SIT bitmap
	bitmap_size = __bitmap_size(sbi, SIT_BITMAP);//bitmap_size = 0x40=64
	src_bitmap = __bitmap_ptr(sbi, SIT_BITMAP);//src_bitmap = 0x155a10c0
//pr_notice("bitmap_size = 0x%x\n", bitmap_size);
//pr_notice("src_bitmap = 0x%x\n", src_bitmap);
	...
	/*5. init SIT information */
	sit_i->s_ops = &default_salloc_ops;

	sit_i->sit_base_addr = le32_to_cpu(raw_super->sit_blkaddr);//0x600 = 1536
	sit_i->sit_blocks = sit_segs << sbi->log_blocks_per_seg;//0x200 = 512
	sit_i->written_valid_blocks = 0;
	sit_i->bitmap_size = bitmap_size;//0x40=64
	sit_i->dirty_sentries = 0;
	sit_i->sents_per_block = SIT_ENTRY_PER_BLOCK;
	sit_i->elapsed_time = le64_to_cpu(sbi->ckpt->elapsed_time);
	sit_i->mounted_time = ktime_get_real_seconds();
	init_rwsem(&sit_i->sentry_lock);
//pr_notice("sit_i->sit_base_addr=0x%x, sit_i->sit_blocks=0x%x,sit_i->bitmap_size=0x%x",sit_i->sit_base_addr, sit_i->sit_blocks,sit_i->bitmap_size);
	return 0;
}
...
static int build_curseg(struct f2fs_sb_info *sbi)
{
	...
	//1. 创建curseg_info结构，分配sum_blk缓存f2fs_summary_blck，分配journal
	for (i = 0; i < NR_CURSEG_TYPE; i++) {
...
{//从设备中读取sit信息放入sit_i->sentires[]中，然后重journal中读取sit信息，保证最新的sit_i->sentries[]
	struct sit_info *sit_i = SIT_I(sbi);
	struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct f2fs_journal *journal = curseg->journal;
	struct seg_entry *se;
...
//pr_notice("sit_blk_cnt = %d\n", sit_blk_cnt);//147
	...
	down_read(&curseg->journal_rwsem);
	//扫描日志，从中恢复更新版本的sit信息到&sit_i->sentries[]中
	for (i = 0; i < sits_in_cursum(journal); i++) {
...
{//根据seg_entry内容设置free_i->free_segments
	unsigned int start;
	int type;
...
{//根据free segmap 和 sit信息，初始化dirty segment信息，即不是所有block都valid的segment
	struct dirty_seglist_info *dirty_i = DIRTY_I(sbi);
	struct free_segmap_info *free_i = FREE_I(sbi);
...
{//创建dirty_seglist_info，根据free_segmap和sit信息填充，每种dirty类型都有一个dirty_segmap
...
{//更新sit_i->min_mtime和max_mtime
	...
	//领整个main area的segments中最小的平均时间作为sit_i->min_mtime
...
/*pr_notice("sm_info->seg0_blkaddr = 0x%x\n, sm_info->main_blkaddr =0x%x\n, sm_info->segment_count = 0x%x\n, sm_info->reserved_segments = 0x%x\n, \
	sm_info->ovp_segments = 0x%x\n, sm_info->main_segments = 0x%x\n, sm_info->ssa_blkaddr = 0x%x\n,  sm_info->rec_prefree_segments = 0x%x\n, \
	sm_info->min_ssr_sections = 0x%x\n ",sm_info->seg0_blkaddr, sm_info->main_blkaddr, sm_info->segment_count, sm_info->reserved_segments,
	sm_info->ovp_segments, sm_info->main_segments, sm_info->ssa_blkaddr,  sm_info->rec_prefree_segments,sm_info->min_ssr_sections);
*/
