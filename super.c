...
{
	loff_t result = 0;
	loff_t leaf_count = ADDRS_PER_BLOCK;//1018
	...
	return result;//2*1018 + 2*1018*1018 +  1018*1018*1018
}
...
/*pr_notice("sbi->log_sectors_per_block = 0x%x\n,	\
	...
	sbi->blocks_per_seg = 0x%x\n, \
	sbi->segs_per_sec =  0x%x\n, \
	sbi->secs_per_zone =  0x%x\n, \
	sbi->total_sections =  0x%x\n, \
	sbi->total_node_count = 0x%x\n, \
	sbi->root_ino_num =  0x%x\n, \
 ...
 * 从f2fs中通过sb_bread读取块0，拷贝到raw_super中
 */
...
{//初始化设备链表，从raw_super.devs[i]拷贝到sbi->devs[i]中
	struct f2fs_super_block *raw_super = F2FS_RAW_SUPER(sbi);
...
//=====================================================================以上为基本初始化============================================
	...
	//分配write_io空间
	// data: sbi->write_io[0] = 3个f2fs_bio_info，冷温热，初始化其中的sbi, io_lock, io_list
	// node: .............[1]...........................................
	// meta: .............[2] = 1个，热
	for (i = 0; i < NR_PAGE_TYPE; i++) {
...
/*pr_notice("sbi->total_valid_node_count = 0x%x\n, \
	sbi->user_block_count = 0x%x\n,\
	sbi->total_valid_block_count = 0x%x\n, \
	sbi->last_valid_block_count = 0x%x\n",sbi->total_valid_node_count,sbi->user_block_count,sbi->total_valid_block_count,sbi->last_valid_block_count);
*/
	...
	if (__exist_node_summaries(sbi)){//进入这个if
		...
		//pr_notice("sbi->kbyts_written = 0x%x\n", sbi->kbytes_written);
		//pr_notice("exist_node_summaries(sbi)\n");
	}
	...
	*/
	root = f2fs_iget(sb, F2FS_ROOT_INO(sbi));	//3
	if (IS_ERR(root)) {
...
{
//pr_notice("flags = 0x%x, dev_name= %s, data = %s\n", flags, dev_name, (char *)data);
// flags = 0x0, dev_name= /dev/nvme0n1, data = (null)
