	...
	bio_for_each_segment_all(bvec, bio, i) {
		...
		if (IS_DUMMY_WRITTEN_PAGE(page)) {
			...
			if (unlikely(bio->bi_status)){
				pr_notice("bio->bi_status = %d",bio->bi_status);
				...
				block_t blk_addr, struct bio *bio)
{
	...
	if (bio) {
		...
		bio->bi_iter.bi_sector = SECTOR_FROM_BLOCK(blk_addr);//根据blk_addr计算sector地址
	}
	return bdev;
}
...
{//创建bio，根据blk_addr填充bi_sector
	struct bio *bio;

	bio = f2fs_bio_alloc(sbi, npages, true);//创建带有napges个iovec的bio

	f2fs_target_device(sbi, blk_addr, bio);//根据blk_addr填充bio中的bio->bi_iter.bi_sector其实sector号
...
{
//pr_notice("Enter __submit_merged_bio()\n");
#ifdef AMF_META_LOGGING	
...
//pr_notice("Enter f2fs_submit_page_bio()\n");
	...
	/* Allocate a new bio */
	//根据new_blkaddr,io_wbc等创建1个page的bio，填充部分字段
	bio = __bio_alloc(fio->sbi, fio->new_blkaddr, fio->io_wbc,
				1, is_read_io(fio->op));

	if (bio_add_page(bio, page, PAGE_SIZE, 0) < PAGE_SIZE) {//向bio添加fio->page
		bio_put(bio);
		return -EFAULT;
	}
				...
				enum iostat_type io_type)
{
	...
	if (unlikely(f2fs_cp_error(sbi))) {
		pr_notice("f2fs_cp_error()\n");
		mapping_set_error(page->mapping, -EIO);
		goto out;
	}

	if (unlikely(is_sbi_flag_set(sbi, SBI_POR_DOING))){
		pr_notice("is_sbi_flag_set\n");
		goto redirty_out;
		}
