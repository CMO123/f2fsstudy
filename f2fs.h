...
struct f2fs_nm_info {
	...
	nid_t max_nid;			/* maximum possible node ids  ‭4193280‬*/
	...
	unsigned int nat_blocks;	/* # of nat blocks =9216*/
	...
	unsigned char *nat_block_bitmap;//用于记录nat_block是否有效的bitmap，空的和满的都是有效的？
...
struct f2fs_sb_info {
	...
	int valid_super_block;			/* valid super block no = 0 */
	...
	loff_t max_file_blocks;			/* max block index of file 由inode中索引决定，2个直接，2间接，1三级 = 2*1018+2*1018*1018+1018*1018*1018 */
	int active_logs;			/* # of active logs = 6 */
	int dir_level;				/* directory level */
	int inline_xattr_size;			/* inline xattr size = 50 */
	...
	struct f2fs_mount_info mount_opt;	/* mount options挂载选项都在这里 */
...
{//从checkpoint处得到NAT或SIT bitmap信息
...
{//从mapping这个cache中找到index offset所在的page，如果没有则创建
#ifdef CONFIG_F2FS_FAULT_INJECTION
	struct page *page = find_lock_page(mapping, index);//从address_space中找到offset为index的pagecache页
...
{//创建一个bio，其中包含npages个iovecs(预分配的)
	struct bio *bio;
