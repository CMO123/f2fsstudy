...
//pr_notice("int nr_chnls = nr_luns / dev->geo.nr_luns\n");
//pr_notice("nr_chnls = %d, nr_luns = %d, dev->geo.nr_luns= %d\n",nr_chnls,nr_luns,dev->geo.nr_luns);


	int nr_chnls_mod = nr_luns % dev->geo.nr_luns;
//pr_notice("nr_chnls_mod=%d\n",nr_chnls_mod);
...
//pr_notice("blun=%d\n",blun);
	...
	for (i = 0; i < nr_chnls; i++) {
		...
		if (!lun_offs)
			goto err_ch;
//pr_notice("luns_in_chnls=0x%x\n",luns_in_chnl);
		for (j = 0; j < luns_in_chnl; j++) {
			luns[lunid].ppa = 0;
			luns[lunid].g.ch = i;
			luns[lunid++].g.lun = j;
//pr_notice("luns[%d]=0x%llx\n",lunid-1,luns[lunid-1].ppa);
			//这边luns只设置了ch和lun的值，为luns[0]=0x0，luns[1]=0x1000000000000，luns[2]=0x2000000000000
			lun_offs[j] = blun;
//pr_notice("lun_offs[%d]=0x%x\n",j,blun);
			lun_roffs[j + blun] = blun;
//pr_notice("lun_roffs[%d]=0x%x\n",j+blun,blun);
...
//pr_notice("tgt_dev->geo.nr_chnls = %d, dev->geo.nr_chnls=%d\n",tgt_dev->geo.nr_chnls, dev->geo.nr_chnls);
	tgt_dev->geo.all_luns = nr_luns;
//pr_notice("tgt_dev->geo.all_luns=%d\n",tgt_dev->geo.all_luns);
	tgt_dev->geo.nr_luns = (lun_balanced) ? prev_nr_luns : -1;
//pr_notice("tgt_dev->geo.nr_luns=%d\n",tgt_dev->geo.nr_luns);
	tgt_dev->geo.op = op;
//pr_notice("tgt_dev->geo.op=%d\n",tgt_dev->geo.op);
	tgt_dev->total_secs = nr_luns * tgt_dev->geo.sec_per_lun;
//pr_notice("tgt_dev->total_secs=%d\n",tgt_dev->total_secs);
...
//	struct request_queue *tqueue;
	struct gendisk *tdisk;//gendisk相关的都注释掉
	struct nvm_tgt_type *tt;
	struct nvm_target *t;
...
/*
	tqueue = blk_alloc_queue_node(GFP_KERNEL, dev->q->node);*//*在指定node上分配请求队列*/ 
/*	if (!tqueue) {
		ret = -ENOMEM;
		goto err_disk;
	}
	blk_queue_make_request(tqueue, tt->make_rq);//使用tt_make_rq作为tqueue的make_request
*/
...
/*	tqueue->queuedata = targetdata;
	...
	blk_queue_max_hw_sectors(tqueue, 8 * dev->ops->max_phys_sect);//dev->ops->max_phys_sect=64
	//为此队列的request设置最大sector数目
pr_notice("dev->ops->max_phys_sect=%d\n",dev->ops->max_phys_sect);
 ...
 * <0: on error
 */
int nvm_remove_tgt(struct nvm_dev *dev, struct nvm_ioctl_remove *remove)
{
	struct nvm_target *t;

	mutex_lock(&dev->mlock);
pr_notice("remove->tgtname = %s\n",remove->tgtname);
	t = nvm_find_target(dev, remove->tgtname);
	if (!t) {
		mutex_unlock(&dev->mlock);
pr_notice("can't find tgtname\n");
		return 1;
	}
...
{//将nvm_tgt_type添加到nvm_tgt_types链表中
	int ret = 0;
...
pr_notice("Enter nvm_core_init()\n");
	...
	blk_queue_logical_block_size(dev->q, geo->sec_size);//设置队列的逻辑块大小
	return 0;
err_fmtype:
	kfree(dev->lun_map);
	return ret;
...
{
pr_notice("Enter nvm_register()\n");
	int ret;

	if (!dev->q || !dev->ops)
		return -EINVAL;

	if (dev->ops->max_phys_sect > 256) {//dev->ops->max_phys_sect=64
...
EXPORT_SYMBOL(__nvm_configure_create);
...
{
pr_notice("Enter nvm_ioctl_dev_create()\n");
	struct nvm_ioctl_create create;

	if (!capable(CAP_SYS_ADMIN))//权限检查
		return -EPERM;

	if (copy_from_user(&create, arg, sizeof(struct nvm_ioctl_create)))//从用户空间拷贝参数到内核空间
		return -EFAULT;
...
static int __init my_misc_device_init(void){
	return misc_register(&_nvm_misc);
}
