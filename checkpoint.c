...
{//向底层发送读取cp_addr地址的命令，读取checkpont，放入cp_block和cp_page中，计算checkpoint的crc是否有效，将checkpoint的version号填入version中

	unsigned long blk_size = sbi->blocksize;
	size_t crc_offset = 0;
	__u32 crc = 0;

	*cp_page = get_meta_page(sbi, cp_addr);//2048, 2560
...
{//根据起始的cp_addr，读取出checkponit内容，返回给page，并填充version
	...
	//pr_notice("cp_addr = %d\n", cp_addr2);
	//err = get_checkpoint_version(sbi, cp_addr2, &cp_block,&cp_page_2, version);
	...
	if (cur_version == pre_version) {//当头部和尾部版本相同时，被认为有效
		*version = cur_version;
